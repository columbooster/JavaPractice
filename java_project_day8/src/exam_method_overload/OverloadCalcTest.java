...
public class OverloadCalcTest {
	...
	public static void main(String[] args) {
		...
		System.out.println("\n가변 길이 메소드 호출에 의한 합 : " + num + "\n");
		
		int[] values1 = {1,2,3};
		int result = over.plusInt(values1); //plusInt(배열의 시작 주소값을 인자값)
		System.out.println("\nint 배열을 매개변수로 갖는 메소드 호출에의한 합(1) : " + result);
		
		result = over.plusInt(new int[] {1,2,3,4,5,}); //plusInt(배열의 시작 주소값을 인자값)
		System.out.println("\nint 배열을 매개변수로 갖는 메소드 호출에의한 합(1) : " + result);
