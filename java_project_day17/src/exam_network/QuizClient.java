...
public class QuizClient {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			out = new PrintWriter(quizSocket.getOutputStream(), true); // 출력스트림(서버에서 클라이언트로 데이터전송)
			in = new BufferedReader(new InputStreamReader(quizSocket.getInputStream())); // 입력스트림(서버로부터 데이터를 클라이언트로 읽어드림)
